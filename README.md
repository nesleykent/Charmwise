...
- **Creature Products and corpse actions** - [`src/data/creatureProducts.json`](src/data/creatureProducts.json), [`src/data/skinning.json`](src/data/skinning.json), and [`src/data/dusting.json`](src/data/dusting.json) are separate from the Bestiary because loot/product mechanics have different source quality and update cadence. Skinning covers the Dragon family, Behemoth, Bonebeast, Clomp, every Lizard variant, and every Minotaur variant in the Bestiary; Dusting covers Vampire and Demon (the only two dustable creatures in the game). Base success chances, where shown, come from community-collected trial counts on TibiaWiki's per-item `/Skinning` and `/Dusting` statistics subpages (e.g. [`Green Dragon Leather/Skinning`](https://tibia.fandom.com/wiki/Green_Dragon_Leather/Skinning)) rather than guessed - each entry cites its own `sourceUrl`. Where a creature is confirmed eligible but no trial sample was found (most Lizard/Minotaur variants individually), the success chance is left `null` rather than inherited from a sibling variant, since per-variant rates in the verified samples differ by up to 3x. Creature Products (Gut) only lists items confirmed as *regular loot* - Green/Red/Albino Dragon Leather and Vampire Dust were removed from this file after checking Dragon's and Dragon Lord's own loot tables, which list those leathers exclusively under "can be skinned with an Obsidian Knife", never under regular Loot; Gut boosts loot rolls, not the separate Skinning/Dusting action, so including them there would have overstated Gut's value for Dragons. Unknown drop or success chances are stored as `null` and excluded from EV rather than guessed.
...
- The product/corpse-action datasets are intentionally partial. Scavenge has verified base success chances for the Dragon family, Behemoth, Bonebeast, and several Lizard/Minotaur variants (sourced from community trial-count statistics, not estimates), plus full Vampire/Dusting and Demon/Dusting coverage - the only two dustable creatures in the game. Gut's regular-loot "Creature Products" category is far larger in TibiaWiki's own reckoning (~970 items across the whole bestiary) and only a verified subset is mapped so far; drop chances for newly-added items like Behemoth Claw and Bear Paw are correctly left unknown rather than guessed, since TibiaWiki itself doesn't publish hard percentages for most loot items.
...
- Expand Creature Product (Gut) coverage further into TibiaWiki's ~970-item catalogue, and fill in the remaining Lizard/Minotaur variants' Skinning success chances (currently `null` where no community trial sample was found) plus world-specific market prices, so Gut and Scavenge can be scored more precisely across more creatures.
